...
select student.name, count(exam_management.number_of_exam_taking) 
...
select student.name, exam_management.mark from student, exam_management
...
select student.name, faculty.name, age
...
and (select max(current_year - to_number(to_char(student.birthday, 'YYYY')))from student);
-- 5. Cho biết khoa nào có đông sinh viên nhất
select faculty.name, count(student.id) from faculty, student    
where faculty.id = student.faculty_id group by faculty.name having count(student.faculty_id)>= all(select count(student.id) from student group by student.faculty_id);
